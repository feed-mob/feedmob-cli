...
- **Formula 收进本仓库，独立 tap 仓库废弃。** `brew tap` 的 `homebrew-*` 命名约定只在省略 URL 时生效；显式 URL 形式（`brew tap feed-mob/tap https://github.com/feed-mob/feedmob-cli.git`）可 tap 任意仓库。因此 `Formula/fm.rb` 直接放本仓库，Job F 用 `GITHUB_TOKEN` 在本仓库开 `release/fm-<version>` PR——跨仓库凭据问题消失，PAT 不再需要。后续仓库积累足够 notability（≥30 stars/forks/watchers）后可评估提 homebrew-core。
